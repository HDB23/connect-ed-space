      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            <Card className="p-6 gradient-card border-0 shadow-medium">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-medium text-success">AI Processing Complete</span>
              </div>
              ...
              <div className="space-y-3">
                <Button className="w-full gradient-primary text-primary-foreground hover-scale transition-smooth">
            ...
            <Card className="p-6 gradient-card border-0 shadow-medium">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary" className="bg-success/10 text-success">98.5%</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-foreground-muted text-sm">Key Points Extracted</span>
                  <Badge variant="secondary" className="bg-primary/10 text-primary">24</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-foreground-muted text-sm">Study Questions</span>
                  <Badge variant="secondary" className="bg-warning/10 text-warning">15</Badge>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Note Content */}
            <Card className="gradient-card border-0 shadow-medium min-h-[500px]">
              <div className="p-6">
                  ...
                  {activeTab === "keypoints" && (
                    <div className="space-y-3">
                      {aiNotes.keypoints.content.map((point, index) => (
                        <div 
                          key={index}
                          className="flex items-start gap-3 p-4 bg-background-subtle rounded-lg hover:bg-background-muted transition-smooth"
                        >
                          <div className="w-6 h-6 rounded-full gradient-primary flex items-center justify-center text-white text-xs font-bold flex-shrink-0 mt-1">
                            {index + 1}
                          </div>
                  ...
                  {activeTab === "questions" && (
                    <div className="space-y-3">
                      {aiNotes.questions.content.map((question, index) => (
                        <div 
                          key={index}
                          className="flex items-start gap-3 p-4 bg-background-subtle rounded-lg hover:bg-background-muted transition-smooth hover-lift"
                        >
                          <div className="w-6 h-6 rounded-full bg-secondary/20 text-secondary flex items-center justify-center text-xs font-bold flex-shrink-0 mt-1">
                            Q{index + 1}
                          </div>
                          <div className="flex-1">
                            <p className="text-foreground leading-relaxed mb-2">{question}</p>
                            <Button variant="outline" size="sm" className="text-xs hover-scale transition-smooth">
                              Generate Answer
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { icon: Wand2, title: "Auto-highlight", desc: "Mark important sections", color: "primary" },
                { icon: Lightbulb, title: "Generate Quiz", desc: "Test your knowledge", color: "secondary" },
                { icon: TrendingUp, title: "Study Plan", desc: "Personalized schedule", color: "success" }
              ].map((action, index) => (
                <Card 
                  key={action.title}
                  className="p-4 gradient-card border-0 shadow-soft hover:shadow-medium hover-lift transition-smooth cursor-pointer"
                >
